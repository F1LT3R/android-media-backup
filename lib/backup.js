...
const sha = (filepath, file) => new Promise((resolve, reject) => {
	...
	rs.on('end', () => {
		...
		console.log(`${filepath}: ${file.hash}`)
		resolve(file.hash)
	})
})
...
module.exports = settingsObject => {
	...
	const createBackupList = collections => new Promise((resolve, reject) => {
		...
		const backupList = []
		const hashes = media.concat(backup).map(file => file.hash)

		Promise.all(hashes).then(results => {
